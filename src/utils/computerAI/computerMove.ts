...
export function makeComputerMove(gameState: GameState, gridSize: number): Point | null {
  ...
  // Ensure we only generate points within the grid bounds
  const availablePoints = generateAvailablePoints(gameState, gridSize - 2); // Subtract 1 to get correct grid size
